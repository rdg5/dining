...
const createNewrole = __importStar(require("../../domain/role-use-case"));
let axiosAPIClient;
beforeAll(async () => {
...
describe('/api', () => {
    describe('POST /roles', () => {
        test('When asked to save a new role, Then it saves it and returns the newly created user', async () => {
            ...
            const postResponse = await axiosAPIClient.post(`/api/roles`, newroleToBeCreated);
        ...
        test('When role creation fails, then it should return status 404', async () => {
            const createNewrolestub = sinon_1.default
